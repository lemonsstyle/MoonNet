...
# Note that we have lost the final version of the hyperparameters.
# You may adjust the parameters below to reproduce the results from the paper.
s_all = {'Family':2, 'Francis':4, 'Horse':2, 'Lighthouse':3, 'M60':2, 'Panther':2, 'Playground':3, 'Train':3}
conf_all = {'Family':0.35, 'Francis':0.5, 'Horse':0.2, 'Lighthouse':0.8, 'M60':0.4, 'Panther':0.3, 'Playground':0.3, 'Train':0.5}
dist_all = {'Family':1/8, 'Francis':1/4, 'Horse':1/2, 'Lighthouse':1/4, 'M60':1/4, 'Panther':1/4, 'Playground':1/4, 'Train':1/4}
rel_diff_all = {'Family':1/1500, 'Francis':1/1500, 'Horse':1/1000, 'Lighthouse':1/1000, 'M60':1/1600, 'Panther':1/1500, 'Playground':1/1600, 'Train':1/1600}
fusion_view = {'Family':11, 'Francis':11, 'Horse':11, 'Lighthouse':11, 'M60':11, 'Panther':11, 'Playground':11, 'Train':11}
...
parser.add_argument('--testpath', default="your_tnt_path/intermediate/", help='testing data dir for some scenes')
...
parser.add_argument('--fusion_views', type=int, default=11, help='num of view')
...
def check_geometric_consistency(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src,scan):
    ...
    for i in range(s,fusion_view[scan]):
...
def save_scene_depth(testlist):
    ...
    if args.dataset == 'tanks' or args.dataset == 'tanks_vit':
    ...
    # model
    model = MonoMVSNet(arch_mode=args.arch_mode, reg_net=args.reg_mode, num_stage=4,
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            outputs = model(sample_cuda["imgs"], sample_cuda["imgs_raw"], sample_cuda["proj_matrices"], sample_cuda["depth_values"])
            end_time = time.time()
            ...
            for filename, cam, img, depth_est, photometric_confidence in zip(filenames, cams, imgs, \
                ...
                H, W = confidence_list[-1].shape
                confidence_list[0] = cv2.resize(confidence_list[0], (W, H))
                confidence_list[1] = cv2.resize(confidence_list[1], (W, H))
                confidence_list[2] = cv2.resize(confidence_list[2], (W, H))
