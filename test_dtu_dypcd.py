...
parser.add_argument('--dataset', default='general_eval_vit', help='select dataset')
parser.add_argument('--testpath', default='you_dtu_test_path', help='testing data dir for some scenes')
...
def check_geometric_consistency(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src,confidence):
    width, height = depth_ref.shape[1], depth_ref.shape[0]
    x_ref, y_ref = np.meshgrid(np.arange(0, width), np.arange(0, height))
...
def filter_depth(pair_folder, scan_folder, out_folder, plyfilename):
    ...
    for ref_view, src_views in pair_data:
        ...
        for src_view in src_views:
            ...
            masks, geo_mask, depth_reprojected, x2d_src, y2d_src,x2d_reprojected,y2d_reprojected = check_geometric_consistency(ref_depth_est, ref_intrinsics,
...
if __name__ == '__main__':
    ...
    save_depth(testlist)
    # step2. fusion all depth maps for generating point clouds
